...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        p = self.place
        dist = 0
        if p.bees and not p.is_hive:
            return random_bee(p.bees)
        while p.entrance:
            dist += 1
            if not p.entrance.is_hive and p.entrance.bees:
                if self.lower_bound <= dist <= self.upper_bound:
                    return random_bee(p.entrance.bees)
            p = p.entrance
        return None
        # END Problem 3 and 4
